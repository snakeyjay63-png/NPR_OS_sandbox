# Document 256: Linux als extrinsiek wiskundig routeveld

**ID:** `npr:doc:256`  
**Route:** `noise → pattern → return`  
**Status:** ✅ canoniek  
**Verwante documenten:** 06 (signaalblok), 18 (sandbox router), 257 (RAM)  
...
```text
256 = 2⁸ = toestandruimte van één byte
```

Dit document draagt nummer 256 omdat Linux bouwt vanuit het byteveld:

```text
binary → byte_256 → hex → adres → interface → route → verbinding
```

```text
DOCUMENT_256 := Linux als extrinsiek operationeel byteveld
```
...
|---|---|---|
| 1 | 2026-07-18 | Initiële stap (nummer 26). |
| 2 | 2026-07-18 | Herkend naar document 256. `256 = 2⁸` = toestandruimte van één byte. Nummer is inhoud, niet sequentie. |
